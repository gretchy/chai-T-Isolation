...
public class Board {
	...
	// setting the game board to starting values and positions of X and O
	...
	// edits the game board to reflect new valid moves made by current player
	public void makeMove(String player, String move) {
		int[] playerLoc = getPosition(player); // stores player's current location on board
		...
		board[playerLoc[0]][playerLoc[1]] = "#"; // old position is marked with a # symbol

		// printBoard();
	}

	// gets the value at a certain location on board
	...
	public void printBoard() {
		...
		for (int row = 0; row < board.length; row++) {
			...
			else { // no one has made any moves yet
